...
- **PDF viewer**: an `<iframe>` PDF only works in desktop browsers (Android Chrome doesn't render it; iOS Safari shows only page 1). Phones and tablets therefore use PDF.js's own viewer components: **all pages scroll continuously downwards** (opening a cited page jumps straight to it), **pinch-to-zoom** (or the +/− buttons; 1x-4x of the screen width), and **search inside the PDF** (magnifier button: type, then Enter or ▲/▼ to move between matches; all matches are highlighted, with a "3/12" counter). Text can also be selected and copied. Desktop keeps the browser's built-in viewer (search, select, print). The library lives in `app/static/vendor/pdfjs-<version>/` and is downloaded only when a PDF is first opened (about 650 KB gzipped, then cached for a year); see `VERSION.txt` there for how to upgrade. Append `?viewer=pdfjs` to the URL to force the PDF.js viewer on desktop for testing.
...
- **In-PDF search only finds real text.** The cover page of `ADART-OSUIMahawaditra-2022.pdf` (page 1) is an image without a text layer, so it can't be searched; every other page of every current document has text. Search works inside the open document only (searching across documents is the chatbot's job). Every viewer has an "open original PDF" button as the fallback.
