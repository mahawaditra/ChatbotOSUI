...
`GEMINI_API_KEY`, `UPSTASH_VECTOR_REST_URL`, `UPSTASH_VECTOR_REST_TOKEN`, `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN` (a separate Upstash Redis database, used only for rate-limiting `/api/chat` — not the same database as the Vector ones above), `ADMIN_KEY` (must be ≥32 chars or `app/config.py` raises `RuntimeError` at startup), and optional `COLLECTION_NAME` (default `org-rag`, currently unused — `get_vector_index()` in `retrieval.py`/`indexing.py` connects with just the Upstash URL/token, no namespace param, so changing this env var has no effect yet). `app/config.py` reads these eagerly at import time via `os.environ[...]` (not `.get`) for the required ones — a missing var crashes on startup, not at request time.
